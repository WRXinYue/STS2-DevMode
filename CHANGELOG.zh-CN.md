...
- 对局内 **存档/读档** 覆盖层：**指定种子重开**：可将卡牌、遗物、金币（可选）带入新对局并输入新种子（通过 `NGame.StartNewSingleplayerRun` 注入种子以保证确定性）。
- **主菜单开发者模式**：**指定种子新建**，与其他 DevMode 面板风格一致。
- **存档管理** 重构：按磁盘动态发现槽位，支持 **新建槽位**、**删除**（二次确认）、左侧槽位卡片（HP 条、楼层/金币等）、右侧详情与徽章式属性、可滚动卡牌/遗物/Mod 列表。
- 存档元数据增加 **对局种子** 与 **已加载 Mod（名称与版本）**，并在详情中展示。
- **`dmsave` 控制台**：新增 `list`、`delete` 子命令；补全基于当前存在的槽位，不再使用固定数量上限。

### Changed

- 存档界面不再单独突出「快速存档」槽位，各槽位平权（控制台 `dmsave quick` 仍写入槽位 0 以方便快捷键等场景）。
- 「指定种子重开」中卡牌/遗物默认 **关闭**。

### Fixed

- 修复角色选择流程清空 `DebugSeedOverride` 导致种子不生效的问题。
- 修复对局结束清理时过早清空待重开状态，导致预设/种子未应用到新对局的问题。
- 修复反复打开读档界面时因静态引用指向已释放节点而触发的 `ObjectDisposedException`。
