...
**前提：** 运行时须加载 DevMode。编译时引用 `DevMode.dll` 即可，**不要**把 DevMode 打进自己的 mod 包。
...
桥接 DLL 须针对最新 `DevMode.dll` 编译（`dotnet build` 后的 `build/DevMode/DevMode.dll`）。以独立 mod 发布，`dependencies` 只需声明内容 mod（DevMode 仅运行时依赖）。
